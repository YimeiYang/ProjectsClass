...
def is_all_caps(s):
    ...
    False
    '''
    #check whether the length of the input is larger than 1
    if(len(s)>1):
        for i in range(len(s)):
            #check whether each element is capitalized
            if not is_capitalized(s[i]):
                    return False
        return True
    else:
        return False
...
def word_to_braille(word):
    ...
    result = ""
    every_char= ""
    #check whether all elements are capitalized letters
    if is_all_caps(word):
        for i in range(len(word)):
            every_char = every_char + char_to_braille(word[i])+"\n"+"\n"
        #Add two CAPITAL in front of the new string
        result = CAPITAL +"\n"+ "\n"+ CAPITAL+"\n"+ "\n" + every_char

    else:
        for j in range(len(word)):
            #check whether the element is a capitalized letter
            if is_capitalized(word[j]):
                #Add one CAPITAL in front of the letter
                result = result + CAPITAL+"\n"+ "\n" + \
                               char_to_braille(word[j]) + "\n"+ "\n"
            #chek whether the element is a number
            elif is_digit(word[j]):
                #Add one NUMBER in front of the letter
                result = result + NUMBER +"\n"+ "\n"+ \
                               char_to_braille(word[j]) + "\n"+ "\n"
            else:
                result = result + char_to_braille(word[j])+ "\n"+"\n"
    return result
...
def new_filename(fname, addition):
    ...
    if ".txt" in fname:
        #get ride of .txt and add addition
        new = fname.replace(".txt", "_"+addition)
        full = new + ".txt"
    #check whether it is a .csv file
    elif ".csv" in fname:
        #get ride of .csv and add addition
        new = fname.replace(".csv", "_"+addition)
        full = new + ".csv"
