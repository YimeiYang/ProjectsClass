...
def convert_irregular(c):
    ...
    '..\\noo\\noo'
    '''
    #check whether the input is a irregular character
    if (is_irregular(c)):
        #check whether it is a apostrophe
        if(c == "`" or c== "’" or c== "'"):
            return APOSTROPHE;
        #check whether it is a hyphen
        elif(c == "–" or c== "-"):
            return HYPHEN;
        #check whether it is a space
        elif(c == " "):
            return SPACE;
        #check whether it is a qoute
        elif(c == "«" or c== "»"):
            return QUOTES;
...
def convert_digit(c):
    ...
    if(is_digit(c)):
        #combine the regular bases with ".."
...
def decade_ending(dec_num):
    ...
    while(is_digit(str(dec_num))):
        ...
        #check whether the input is 1
            ...
            return "o.";
        #check whether the input is 2
        elif(dec_num == 2):
            return "oo";
        #check whether the input is 3
        elif(dec_num == 3):
            return ".o";
    return None;
...
def convert_letter(c):
    ...
    #check whether the input is a digit
    if(is_letter(c)):
        #combine the regular bases for colums and rows together
...
def char_to_braille(c):
    ...
    '\\n'
    '''
    #to check whether the input is a known character
    if(is_known_character(c)):
        #to check whether the input is a irregular letter
        if(is_irregular(c)):
            return convert_irregular(c);
        #to check whether the input is a digit
        elif(is_digit(c)):
            return convert_digit(c);
        #to check whether the input is a punctuation
        elif(is_punctuation(c)):
            return convert_punctuation(c);
        #to check whether the input is a letter
        elif(is_letter(c)):
